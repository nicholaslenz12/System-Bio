...
def PcNull(R):
    ...
    """
    return S*(r2*(K2**B2/(K2**B2 + A**B2))*(R*M)/(K3 + R*M) - gc)/((1+R)*gc)
...
def RNull(R):
    ...
    return -S*R*(r1*(K1**B1/(K1**B1 + (A/R)**B1)) - r2*(K2**B2/(K2**B2 + (A)**B2)) \
        *(R*M)/(K3 + R*M))/((-gwt*A + R*gc)*(R+1)) + S/(R+1)
...
Rrange = np.linspace(0.1, R_max, 10000, endpoint=False)
Rrange_asymptote = np.linspace(small_float, A*gwt/gc, 10000, endpoint=False)

# Mesh for vector field
Rsteps,Pcsteps = np.meshgrid(
    np.linspace(0.1, R_max, granularity, endpoint=False),
    np.linspace(0.1, Pc_max, granularity, endpoint=False))
